...
- Sources can be created from `/admin/sources/new`, edited, deleted, opened in a new tab, and manually marked checked. Each checklist item exposes `Edit`, `Mark Checked`, and `Open Source`; marking checked removes it immediately from Pending. `/admin/sources` defaults to a Pending view showing sources never checked or not checked in the past 3 days, with a dropdown `Show all` filter for the full list.
