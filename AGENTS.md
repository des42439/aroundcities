...
- The generic `/admin/feeds` hub redirects to `/admin/events`; deeper feed-backed routes remain available where Events and internal editing flows still need them.
- The `/admin` workflow hub shows counts for Events, Sources, and other visible admin sections.
...
- Leads import uses `aroundcities_leads_import_v1`, always creates active leads, and does not support export in v1. The canonical array key is `items`; older newspaper-task JSON using `records`, `description`, and `place_hint` is also accepted.
